...
const IntervalInnerError = time.Minute    // 内部错误最小发送间隔
...
type Service struct {
	...
	lastInnerErrorTime time.Time // 上一次内部错误发送时间
}
...
func (s *Service) onSendNotify(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if err == nil || err == io.EOF {
			...
			if err != nil {
				s.onInnerErrorWithNotify(logPrefix, "http请求解析错误, content=%s, url=%s, err=%s", string(body[:n]), r.URL, err.Error())
		...
		func() {
			...
			for _, k := range keys {
				...
				if now.Sub(snd.lastAliveTime) > IntervalKeep {
					...
					delete(s.senders, k)
					s.senders2 = util.SliceRemove(s.senders2, snd)
				}
			}
		}()
...
func (s *Service) onInnerErrorWithNotify(prefix, format string, a ...interface{}) {
	...
	now := time.Now()
	if now.Sub(s.lastInnerErrorTime) > IntervalInnerError {
		mobs := s.getAdminMobs()
		mobsn := make([]int64, 0)
		for _, v := range mobs {
			if n, ok := util.String2Int64(v); ok {
